...
const slotsInRing = (baseSize: number, ring: number): number => {
  ...
  if (ring === 3) { return slotsInRing(baseSize, ring - 1) + (bridges ? 3 * podLines : 5 * podLines) }
  if (ring === 4) { return slotsInRing(baseSize, ring - 1) + (bridges ? 5 * podLines : 5 * baseSize) }
  return slotsInRing(baseSize, 4) + (ring - 4) * (bridges ? 5 * podLines : 5 * baseSize)
}
...
/**
     ...
     */
const nextDockPositions = (baseSize: number, rings: Ring[]): { left: Dock, right: Dock }[] => {
  if (rings.length > 10) {
  ...
  if (bridges && rings.length === 5) {
    ...
    return secondPods.flatMap((secondPod, idx) => [secondPod, bridgeSecondPods[idx]])
      .map(component => {
        return ({
...
const addRings = (slots: number, baseSize: number, rings: Ring[]): Ring[] => {
  ...
  const nextRingSlots = Math.min(slots, slotsInRing(baseSize, completeRings + 1)) - slotsInRing(baseSize, completeRings)
  ...
  const nextRing: Ring = dockPositions.flatMap(({ left, right }, groupNum): Ring => {
    //slots in this group
    ...
    const priorSlots = slotsInRing(baseSize, completeRings) + //number of slots in prior rings
 ...
 */
export default function buildFormation(slots: number, baseSize: number): Formation {
  return new FormationImpl(_.flatten(addRings(slots, baseSize, [[new Base(baseSize)]])))
}