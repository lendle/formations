...
import { Plane, Formation, Slot, FormationSlot, NumDict } from "../formation/interfaces";
...
const stringRange = (stop: number): string[] => d3.range(stop).map(x => x.toString())
...
const translate = (d: SlotData) => `translate(${d.offset.scale(SCALE_FACTOR).x},${d.offset.scale(SCALE_FACTOR).y})`
 ...
 */
const addSlot = (g: d3.Selection<SVGGElement, SlotData, SVGGElement, {}>, fill: SlotDataFun, label: SlotDataFun) => {
    g.append("path")
        .attr("class", "grips")
        .attr("d", arc)
...
const transitionSlot = (
    ...
    const changed = (d: SlotData, i: number, nodes: ArrayLike<SVGGElement>):boolean => {
    ...
    const delays = changedSlots.sort().reduce((acc, s, i) => { acc[s] = i; return acc }, {} as NumDict<number>)
...
export default class FormationDrawer extends AbstractDrawer<FormationArgs> {

    draw({ formation,
        planes,
        slots,
        viewConfig: { colorBy, numberBy } }: FormationArgs): void {

        const slotData: SlotData[] = slots.map(({ formationSlotId, planeId }) => ({
        ...
        const buildOrderFill = (d: SlotData) => d3.scaleOrdinal(d3.schemePaired)
            .domain(stringRange(numColors))((d.buildOrder - 1).toString())
        const planeFill = (d: SlotData) => d3.scaleOrdinal(d3.schemePaired.filter((_, i) => [0, 2, 4].includes(i)))
            .domain([PlanePosition.LEAD, PlanePosition.LT, PlanePosition.RT])(d.plane)
        ...
        const slotGroups = this.grp.selectAll<SVGGElement, SlotData>("g.slot")
            .each((d, i, nodes) => { oldData.set(nodes[i], d) })
        ...
        slotGroups.exit().transition(t as any)
            .attr("transform", "translate(0,0) scale(0)")
            .remove()
